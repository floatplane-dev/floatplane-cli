...
if [ -z "$pass_god" ]
then
  echo "Aborting, please provide a pass_god."
  exit 0;
fi

if [ -z "$pass_bot" ]
then
  echo "Aborting, please provide a pass_bot."
  exit 0;
fi
...
useradd --password $temp1 --create-home --groups sudo jw

# Create a user for the deploy bot.
# Do not add to sudo, nor use password.

useradd --password $temp2 --create-home bot
